...
async def test_encode_documents(tmp_path):
    # encode_documents reads images from disk and base64-encodes them, so the
    # test needs a real file on disk. The HTTP call goes through _client.request
    # (not _client.post) since the worker client routes everything through a
    # single retry-aware request method.
